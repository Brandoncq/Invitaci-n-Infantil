...
export const MelissaEduardo: React.FC = () => {
	...
	const IMAGES: Images[] = [
		...
		{ src: "https://picsum.photos/2200/3300", alt: "Random Image 4", class: "!object-top" },
	...
	const CEREMONY_IMAGES = ["https://picsum.photos/500/300?random=1", "https://picsum.photos/500/300?random=2"];
	const RECEPTION_IMAGES = ["https://picsum.photos/500/300?random=3", "https://picsum.photos/500/300?random=4"];
	...
	const SCHEDULE_ITEMS = [
		{
			...
			icon: <ChurchIcon className="w-fit max-lg:self-end self-center object-contain h-[50px]" />,
			isUp: true,
		},
		{
			time: "1:30pm",
			description: "Recepción",
			icon: <ChurchIcon className="w-fit max-lg:self-start self-center object-contain h-[50px]" />,
			isUp: false,
		},
		{
			time: "2:30pm",
	...
	const MODALS_DATA = {
		yape: {
			name: "Melissa Aracely Málaga Zuñiga - Yape",
	...
	const DRESS_CODE_CONFIG = {
		...
		femaleDescription: ["El blanco es exclusivo para la novia", "No colores claros", "(beige, perla, champagne, dorado)"],
	...
	const RESTRICTION_CONFIG = {
		title: "Sólo adultos",
		description: ["Adoramos a sus pequeños, sin embargo este evento está destinado sólo para adultos.", "¡Esperamos tu comprensión!"],
	};
	...
	return (
		<>
			...
			<Privilege
				fontTitle={FONT_TITLE}
				fontDescription={FONT_DESCRIPTION}
				brideParents={[{ name: "Ronald Málaga Vásquez" }, { name: "Rosalvina Zúñiga Olano" }]}
				groomParents={[{ name: "Eduardo Castro Román ✝" }, { name: "Adalguisa Cúneo Toso" }]}
				godparents={[{ name: "Edward Vargas Valderrama" }, { name: "Maritza Dávila Olano" }]}
			/>
			<Celebration
				fontTitle={FONT_TITLE}
				fontDescription={FONT_DESCRIPTION}
