...
export const RecomendationMusic = ({ fontTitle, fontDescription, config }: RecomendationMusicProps) => {
	...
	const handleChangeMusic = (e: any) => {
		...
		setFormData((prevData) => ({
			...
			isMessageValid: name === "message" ? !!value.trim() : prevData.isMessageValid,
		}));
	};
	...
	return (
		<>
			...
			<ModalData
				...
				animationOptions={defaultOptionsChecked}
			>
				<form className={`flex flex-col gap-2 w-[80%] lg:w-[90%] text-[#212121] ${fontDescription}`}>
					<input
						id="name"
						name="name"
						type="text"
					...
					<textarea
						...
						className={`w-full h-28 resize-none border py-2 px-4 hover:bg-[#ceefd5a6] hover:border-[#5a8766a6] text-sm rounded-md focus:outline-none focus:bg-[#ceefd548] placeholder-[#212121] focus:border-[#5a8766a6] ${
							!formData.isMessageValid ? "bg-[#FF8A85a6] border-[#ff4040]" : "bg-[#ceefd548] border-transparent"
						}`}
			...
			<Column
				...
				className={`${window.innerWidth < 1024 && "recomendationmusic-bg"} gap-5 w-full py-7`}
				data-aos="fade-up"
				data-aos-duration="1500"
			>
				<h2 className={`retro-signature-regular text-2xl lg:text-4xl text-[#000000] font-semibold ${fontTitle}`}>Recomienda tu música</h2>
				<Column className={`text-sm lg:text-base w-[80%] lg:w-[30rem] text-center ${fontDescription}`}>
					¿Cuáles son esas canciones que no pueden faltar en nuestra boda? <br /> Comparte tus favoritas con nosotros.
				</Column>
				<button
