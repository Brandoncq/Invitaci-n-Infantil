...
export const GiftCollection = ({ fontTitle, fontDescription, modalsData, accountData, locationData }: GiftCollectionProps) => {
	const [modalType, setModalType] = useState<"yape" | "plin" | "location" | null>(null);
	...
	return (
		...
		>
			<ModalGiftCollection
				...
				accountData={
					modalType === "yape"
						? [{ type: "Número de Yape", number: accountData.yapeNumber }, ...accountData.bankAccounts]
						: modalType === "plin"
						? [{ type: "Número de Plin", number: "1234567890" }, ...accountData.bankAccounts]
						: undefined
				}
			...
			>
				...
				<h2 className={`text-2xl lg:text-4xl retro-signature-custom text-[#000000] font-semibold ${fontTitle}`}>Mesa de Regalos</h2>
				<Column
					mainAxisAlignment="center"
					crossAxisAlignment="center"
					className=" gap-3 lg:gap-5 lg:w-[90%] pr-2"
				>
					<p className={`text-sm leading-relaxed lg:text-base ${fontDescription}`}>
						El mejor regalo es que estés presente en este día tan esperado, pero si deseas hacernos un obsequio tenemos estas opciones:
					</p>
				</Column>
				<Column
					crossAxisAlignment="center"
				...
				>
					<div className={`flex flex-wrap items-center justify-center gap-2 lg:gap-3 ${fontDescription}`}>
						<button
