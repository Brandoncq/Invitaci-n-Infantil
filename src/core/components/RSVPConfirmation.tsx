...
export const RSVPConfirmation = ({ fontTitle, fontDescription, config }: RSVPConfirmationProps) => {
	...
	const handleChange = (e: any) => {
		...
		setFormData((prevData) => ({
			...prevData,
			[name]: value,
			[`is${name.charAt(0).toUpperCase() + name.slice(1)}Valid`]: !!value.trim(),
		}));
	};
	...
	return (
		<>
			...
			<Column
				...
				className={`${window.innerWidth < 1024 && "assistance-assistance"} gap-3 max-lg:pt-10 max-lg:pb-16 w-full`}
				data-aos="fade-up"
				data-aos-duration="1500"
			>
				<h2 className={`text-2xl lg:text-4xl text-[#000000] font-semibold ${fontTitle}`}>Confirma tu Asistencia</h2>
				{config.text && <p className="text-sm lg:text-base w-[80%] lg:w-[30rem]">{config.text}</p>}
				<form className={`flex flex-col gap-2 w-[80%] lg:w-[30rem] text-[#212121] ${fontDescription}`}>
					...
					<select
						...
						className={`w-full cursor-pointer border py-1.5 px-3 hover:bg-[#ceefd5a6] hover:border-[#5a8766a6] text-sm rounded-md focus:outline-none focus:bg-[#ceefd548] placeholder-[#212121] focus:border-[#5a8766a6] ${
							!formData.isConfirmationValid ? "bg-[#ff404033] border-[#ff4040]" : "bg-[#ceefd548] border-transparent"
						}`}
						onChange={handleChange}
					>
						<option
					...
					<textarea
						...
						className={`w-full h-28 resize-none border py-2 px-4 hover:bg-[#ceefd5a6] hover:border-[#5a8766a6] text-sm rounded-md focus:outline-none focus:bg-[#ceefd548] placeholder-[#212121] focus:border-[#5a8766a6] ${
							!formData.isMessageValid ? "bg-[#ff404033] border-[#ff4040]" : "bg-[#ceefd548] border-transparent"
						}`}
